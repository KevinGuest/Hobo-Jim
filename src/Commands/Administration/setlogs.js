...
const loggingConfigFilePath = path.join(dataDirectory, 'userLogsConfig.json');
...
function loadLoggingConfig() {
    ...
    if (fs.existsSync(loggingConfigFilePath)) {
        try {
            ...
            console.error('[UserLogging] Error reading config file:', error);
            return {};
        }
    }
...
module.exports = {
    data: {
        name: 'setlogs',
        description: 'Set the BobbaRP user logs channel for this server.',
        options: [
            {
                name: 'channel',
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            .setTitle('User Logging Channel Set')
            .setColor('#43BA55')
            .setDescription(`✅ User Logging channel has been updated to <#${channel.id}>.`)
            .setTimestamp();
