  ...
  try {
    ...
    for (const [guildId, channelId] of Object.entries(liveFeedConfig)) {
      ...
      if (
        !messageQueue.some(
          (msg) =>
            msg.embed.description === cleanMessage &&
...
client.once("ready", async () => {
  ...
  // Send a restart notification to all configured channels
  for (const [guildId, channelId] of Object.entries(liveFeedConfig)) {
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor("#FFA500") // Orange for restart
        .setDescription(
          "🔄 LiveFeed has been restarted and is now operational."
        );

      await channel.send({ embeds: [embed] });
    } catch (error) {
      console.error(
        `[LiveFeed] Failed to send restart notification to guild ${guildId}:`,
        error
      );
    }
  }
  ...
  client.on("liveFeedUpdate", (guildId, channelId) => {
    console.log(
      `[LiveFeed] Updating live feed channel for guild ${guildId} to channel ${channelId}`
    );
